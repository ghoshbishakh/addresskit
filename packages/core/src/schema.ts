...
function label(id: FieldId, config: CountryAddressConfig): string {
  switch (id) {
    ...
    case "locality":
      return (
        config.localityType.charAt(0).toUpperCase() +
        config.localityType.slice(1)
      );
    case "administrativeArea":
      return (
        config.administrativeAreaType.charAt(0).toUpperCase() +
        config.administrativeAreaType.slice(1)
      );
    case "postalCode":
...
function buildSchema(config: CountryAddressConfig): AddressSchema {
  const fields: Field[] = [];
