...
import { CheckCircle2, AlertCircle, RotateCcw, ChevronLeft, Globe } from "lucide-react";
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-10">
      <div className="mb-6 flex items-center gap-2 text-xs text-muted-foreground">
        ...
        <span>/</span>
        <span className="text-foreground font-medium">libaddressinput</span>
      </div>

      <div className="mb-8">
        <div className="flex items-center gap-2 mb-2">
          <Badge variant="secondary" className="gap-1 text-xs">
            <Globe className="h-3 w-3" /> Data Provider
          </Badge>
          <Badge variant="outline" className="text-xs">256 Countries</Badge>
        </div>
        <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-foreground">
          Google libaddressinput Provider
        </h1>
        <p className="mt-1 text-sm text-muted-foreground max-w-3xl">
          Standard address form driven by Google libaddressinput metadata with country-specific field ordering, required rules, and postal regex patterns.
        </p>
      </div>
            ...
            <Card className="border-border/80">
              <CardHeader className="pb-4">
                ...
                <CardDescription className="text-xs">
                  Fields dynamically adapt based on selected country metadata.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <Address
